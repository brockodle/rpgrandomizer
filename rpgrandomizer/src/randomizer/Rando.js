...
function Randomizertable(){
    ...
    function makerolls(sect) {
        const c1 = rpgresults[Math.round(Math.random()*rpgresults.length)];
    ...
    return (
        <div id='questoutline' className='newrow'>
            A/an <strong><span style={{'color':'#ff7c80'}}>{makerolls('race')} {makerolls('role')}</span></strong> is {makerolls('event')}, so tasks you to {makerolls('action')} {makerolls('noun')} in {makerolls('location')} so that {makerolls('reason')} BUT {makerolls('twist')}!
            <div id='enemykind'><strong>The enemy is {makerolls('enemy')}</strong></div>
            </div>
    )
}
